...
export type PlantsListResponse = {plants: Plant[], total: number};
...
export function getPlantList(
  ...
  return api
    .getPlantList({ skip: 0, ...args })
    .then((responseData) => {
      const total = ( responseData?.plantCollection?.total ?? 0 ) as number;

      return {
        plants: selectors.selectPlants(responseData.plantCollection),
        total: total,
      }
    })
}
